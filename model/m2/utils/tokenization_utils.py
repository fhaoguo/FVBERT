# coding: utf-8
# @Author     :fenghaoguo
# @Time       :2022/5/22 09:37
# @FileName   :tokenization_utils.py
# @Description:
...
class PreTrainedTokenizer(object):
    ...
    Class attributes (overridden by derived classes):
        - ``vocab_files_names``: a python ``dict`` with, as keys, the ``__init__`` keyword name of each vocabulary file required by the m2, and as associated values, the filename for saving the associated file (string).
        - ``pretrained_vocab_files_map``: a python ``dict of dict`` the high-level keys being the ``__init__`` keyword name of each vocabulary file required by the m2, the low-level being the `short-cut-names` (string) of the pretrained models with, as associated values, the `url` (string) to the associated pretrained vocabulary file.
        - ``max_model_input_sizes``: a python ``dict`` with, as keys, the `short-cut-names` (string) of the pretrained models, and as associated values, the maximum length of the sequence inputs of this m2, or None if the m2 has no maximum input size.
    Parameters:
        ...
        - ``cls_token``: (`Optional`) string: a classification token (e.g. to extract a summary of an input sequence leveraging self-attention along the full depth of the m2). Will be associated to ``self.cls_token``
        - ``mask_token``: (`Optional`) string: a masking token (e.g. when training a m2 with masked-language modeling). Will be associated to ``self.mask_token``
    ...
    @property
    def cls_token(self):
        """ Classification token (string). E.g. to extract a summary of an input sequence leveraging self-attention along the full depth of the m2. Log an error if used while not having been set. """
    ...
    @property
    def mask_token(self):
        """ Mask token (string). E.g. when training a m2 with masked-language modeling. Log an error if used while not having been set. """
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if pretrained_model_name_or_path in s3_models:
            ...
            logger.info(
                "Model name '{}' not found in m2 shortcut name list ({}). "
        ...
        if pretrained_model_name_or_path in cls.max_model_input_sizes:
            # if we're using a pretrained m2, ensure the tokenizer
    ...
    def add_tokens(self, new_tokens):
        ...
        Examples::
            # Let's see how to increase the vocabulary of Bert m2 and tokenizer
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
            m2 = BertModel.from_pretrained('bert-base-uncased')
            num_added_toks = tokenizer.add_tokens(['new_tok1', 'my_new-tok2'])
            print('We have added', num_added_toks, 'tokens')
            m2.resize_token_embeddings(len(tokenizer))  # Notice: resize_token_embeddings expect to receive the full size of the new vocabulary, i.e. the length of the tokenizer.
        """
        if not new_tokens:
            return 0
    ...
    def add_special_tokens(self, special_tokens_dict):
        ...
        Examples::
            ...
            m2 = GPT2Model.from_pretrained('gpt2')
    ...
    def convert_tokens_to_ids(self, tokens):
        ...
        if len(ids) > self.max_len:
            logger.warning("Token indices sequence length is longer than the specified maximum sequence length "
                           "for this m2 ({} > {}). Running this sequence through the m2 will result in "
