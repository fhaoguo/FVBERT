# coding: utf-8
# @Author     :fenghaoguo
# @Time       :2022/5/22 10:09
# @FileName   :neuron_view.py
# @Description:
...
#
# Change log
# 12/12/18  Jesse Vig   Adapted to BERT m2
...
def get_attention(model, model_type, tokenizer, sentence_a, sentence_b=None, include_queries_and_keys=False):
    ...
    Args:
        model: pytorch-transformers m2
        model_type: type of m2. Valid values 'bert', 'gpt2', 'xlnet', 'roberta'
    ...
    if model_type not in ('bert', 'gpt2', 'xlnet', 'roberta'):
        raise ValueError("Invalid m2 type:", model_type)
    if not sentence_a:
